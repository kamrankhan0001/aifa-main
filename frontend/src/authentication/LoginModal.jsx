...
//         <button 
//           onClick={onClose} 
//           className="absolute top-6 right-6 text-gray-500 hover:text-white transition-colors p-2 cursor-pointer"
//         >
//           <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg>
//         </button>
...
//               type="submit"
//               disabled={loading || message.type === "success"}
//               className="w-full bg-[#C7E36B] text-black py-4 rounded-xl font-bold mt-2 hover:shadow-[0_0_20px_rgba(199,227,107,0.3)] hover:scale-[1.01] active:scale-[0.99] transition-all disabled:opacity-50 cursor-pointer"
//             >
//               {loading ? "AUTHENTICATING..." : "VERIFY & LOGIN"}
//             </button>
//           </form>
...
//                 <button 
//                 onClick={() => {setView("phone"); clearFeedback();}}
//                 className="flex items-center justify-center gap-2 border border-white/10 text-white py-3 rounded-xl hover:bg-white/5 transition-all text-sm font-medium cursor-pointer"
//                 >
//                 <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path></svg>
//                 Phone
//                 </button>
...
// export default function LoginModal({ onClose, onSwitchToSignup, onSwitchToLogin }) {
//   // --- VIEW CONTROL ---
//   const [view, setView] = useState("email"); // email, forgot-password, reset-password, phone
...
//         if (res.ok) {
//           localStorage.setItem("token", data.token);
//           setFeedback("success", `Welcome back, ${data.name}!`);
//           setTimeout(() => {
//             onClose();
//             navigate("/dashboard");
//           }, 1500);
//         } else {
//           setFeedback("error", data.message || "Google Login failed");
//         }
//       } catch (err) {
//         setFeedback("error", "Failed to connect to auth server");
//       } finally {
//         setLoading(false);
//       }
//     },
...
//     e.preventDefault();
//     clearFeedback();
//     if (!email || !password) return setFeedback("error", "Enter credentials");
...
//       if (response.ok) {
//         localStorage.setItem("token", data.token);
//         setFeedback("success", "Access granted. Redirecting...");
//         setTimeout(() => {
//           onClose();
//           navigate("/dashboard");
//         }, 1500);
...
//   // --- 3. PHONE OTP LOGIC (Manual Generator) ---
//   const handleRequestPhoneOtp = async () => {
//     clearFeedback();
//     if (!phoneNumber) return setFeedback("error", "Enter your phone number");
...
//       if (res.ok) {
//         setShowOtpField(true);
//         setFeedback("success", "OTP sent! Check your server console.");
//       } else {
//         setFeedback("error", "Failed to send OTP");
//       }
//     } catch (err) {
//       setFeedback("error", "Server connection failed");
//     } finally {
//       setLoading(false);
//     }
//   };
...
//     clearFeedback();
//     if (!otp) return setFeedback("error", "Enter the 6-digit OTP");
...
//       if (res.ok) {
//         localStorage.setItem("token", data.token);
//         setFeedback("success", "OTP Verified! Welcome.");
//         setTimeout(() => {
//           onClose();
//           navigate("/dashboard");
//         }, 1500);
//       } else {
//         setFeedback("error", data.message || "Invalid OTP");
//       }
//     } catch (err) {
//       setFeedback("error", "Verification failed");
//     } finally {
//       setLoading(false);
//     }
//   };
...
//   return (
//     <div onClick={onClose} className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 px-4 animate-in fade-in duration-300">
//       <div onClick={(e) => e.stopPropagation()} className="w-full max-w-md bg-[#0B0F10] rounded-3xl p-8 relative border border-white/10 shadow-2xl overflow-hidden">
        
//         {/* Glow Effect */}
//         <div className="absolute -top-24 -left-24 w-48 h-48 bg-[#C7E36B]/10 blur-[100px] rounded-full"></div>
        
//         <button onClick={onClose} className="absolute top-6 right-6 text-gray-500 hover:text-white transition-colors p-2"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg></button>

//         <div className="mb-8">
//           <p className="text-[#C7E36B] text-[10px] uppercase tracking-[0.2em] font-bold mb-1">AIFA FILM ACADEMY</p>
//           <h2 className="text-white text-4xl font-bold tracking-tight">
//             {view === "forgot-password" ? "Reset" : view === "reset-password" ? "Verify" : view === "phone" ? "Mobile" : "Login"}
//           </h2>
//         </div>

//         {message.text && (
//           <div className={`mb-6 p-4 rounded-xl border text-xs font-medium flex items-center gap-3 ${message.type === "success" ? "bg-green-500/10 border-green-500/20 text-green-400" : "bg-red-500/10 border-red-500/20 text-red-400"}`}>
//             <span className="w-1.5 h-1.5 rounded-full bg-current"></span>
//             {message.text}
//           </div>
//         )}
...
//         {view === "email" && (
//           <form onSubmit={handleEmailLogin} className="space-y-4">
//             <input type="email" placeholder="Email Address" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-5 py-4 text-white outline-none focus:ring-2 ring-[#C7E36B]/20 focus:border-[#C7E36B] transition-all" />
//             <input type={showPassword ? "text" : "password"} placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-5 py-4 text-white outline-none focus:border-[#C7E36B] transition-all" />
            
//             <div className="flex items-center justify-between text-xs px-1">
//               <div className="flex items-center gap-2 cursor-pointer" onClick={() => setShowPassword(!showPassword)}>
//                 <div className={`w-4 h-4 rounded border flex items-center justify-center ${showPassword ? 'bg-[#C7E36B] border-[#C7E36B]' : 'border-white/20'}`}>
//                     {showPassword && <svg className="w-3 h-3 text-black" fill="currentColor" viewBox="0 0 20 20"><path d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"></path></svg>}
//                 </div>
//                 <span className="text-gray-400">Show password</span>
//               </div>
//               <span onClick={() => setView("forgot-password")} className="text-gray-500 hover:text-[#C7E36B] cursor-pointer">Forgot Password?</span>
//             </div>

//             <button type="submit" disabled={loading} className="w-full bg-[#C7E36B] text-black py-4 rounded-xl font-bold mt-2 hover:scale-[1.01] transition-all disabled:opacity-50">
//               {loading ? "AUTHENTICATING..." : "VERIFY & LOGIN"}
//             </button>
//           </form>
//         )}
...
//         {view === "phone" && (
//           <div className="space-y-4">
//             <input type="text" placeholder="Phone Number (e.g. +91...)" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-5 py-4 text-white outline-none focus:border-[#C7E36B] transition-all" />
//             {showOtpField && (
//               <input type="text" placeholder="6-digit OTP" value={otp} onChange={(e) => setOtp(e.target.value)} maxLength={6} className="w-full bg-white/5 border border-white/10 rounded-xl px-5 py-4 text-white outline-none text-center tracking-[0.5em] font-bold" />
//             )}
//             <button onClick={showOtpField ? handleVerifyPhoneOtp : handleRequestPhoneOtp} disabled={loading} className="w-full bg-[#C7E36B] text-black py-4 rounded-xl font-bold transition-all">
//               {loading ? "PROCESSING..." : showOtpField ? "VERIFY OTP" : "SEND OTP"}
//             </button>
//             <p onClick={() => {setView("email"); clearFeedback();}} className="text-center text-xs text-gray-500 cursor-pointer hover:text-white">Back to Email</p>
//           </div>
//         )}

//         {view === "forgot-password" && (
//           <form onSubmit={handleResetRequest} className="space-y-4">
//              <input type="email" placeholder="Recovery Email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-5 py-4 text-white outline-none" />
//              <button type="submit" className="w-full bg-[#C7E36B] text-black py-4 rounded-xl font-bold">SEND RESET CODE</button>
//              <p onClick={() => setView("email")} className="text-center text-xs text-gray-500 cursor-pointer">Back to Login</p>
//           </form>
//         )}

//         {/* OR SEPARATOR */}
//         {view !== "reset-password" && view !== "forgot-password" && (
//           <>
//             <div className="flex items-center gap-4 my-8">
//               <div className="flex-1 h-[1px] bg-white/5"></div>
//               <span className="text-gray-600 text-[10px] font-bold tracking-widest">OR CONTINUE WITH</span>
...
//             <div className="grid grid-cols-2 gap-4">
//                 <button onClick={() => {setView("phone"); clearFeedback();}} className="flex items-center justify-center gap-2 border border-white/10 text-white py-3 rounded-xl hover:bg-white/5 transition-all text-sm font-medium">
//                   <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path></svg>
//                   Phone
//                 </button>
//                 <button onClick={() => googleLogin()} className="flex items-center justify-center gap-2 border border-white/10 text-white py-3 rounded-xl hover:bg-white/5 transition-all text-sm font-medium">
//                   <img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" alt="G" className="w-4 h-4" />
//                   Google
//                 </button>
//             </div>
...
//         <p className="text-center text-gray-500 text-xs mt-10">
//           New to AIFA Academy? <span onClick={onSwitchToSignup} className="text-white font-bold hover:text-[#C7E36B] cursor-pointer ml-1">Create Account</span>
//         </p>
//       </div>
//     </div>
//   );
